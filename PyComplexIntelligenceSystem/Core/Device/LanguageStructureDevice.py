"""
@File   ：LanguageStructureDevice.py
@Author ：Ethan Lin
@Date   ：2022/11/04
@Desc   : 语言结构器。存储语言结构数据。
"""
...
@dataclass()
class LanguageStructureDevice(BaseDevice):
    # TODO

    def LexiconDevice(self):
        """
        词项器
        Returns:
...
class LexiconDevice(BaseDevice):
    pass  # class
...
def Lexicon(**rules):
    """创建一个字典符号以转换词语Create a dictionary mapping symbols to alternative words.
...
E0 = Grammar('E0',
             ...
             Lexicon(  # Lexicon for E_0 [Figure 22.3]
                 动作名词="",
                 名词="臭 | 微风 | 闪光 | nothing | wumpus | pit | pits | gold | east",  # noqa
